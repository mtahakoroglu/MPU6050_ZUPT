...
void MPU6050::begin() {
    if (sample_rate_ == 0) {
        sample_rate_ = 100;  // Set a default sample rate if none specified
    }
    begin(sample_rate_);
}

void MPU6050::begin(uint16_t frequency) {
  sample_rate_ = frequency;
...
void MPU6050::update() {
  ...
  if (current_time - last_update_ >= 1000000 / sample_rate_) {
    wire->beginTransmission(MPU6050_ADDR);
    wire->write(0x3B);
    wire->endTransmission(false);
